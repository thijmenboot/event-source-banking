...
impl EventStore for EventStoreSqlite {
    fn append_event<T, E: Event<T> + Serialize>(&self, aggregate_id: Ulid, aggregate_type: &str, event: E) -> Result<(), String> {
        ...
        let envelope = EventEnvelope::new(sequence_number, aggregate_id, aggregate_type.to_string(), event.event_type().to_string(), event);
        ...
        let mut statement = conn.prepare("INSERT INTO events (sequence_number, aggregate_id, aggregate_type, event) VALUES (:sequence_number, :aggregate_id, :aggregate_type, :event)")
    ...
    fn get_events_for_aggregate<T, E: Event<T> + Serialize + for<'de> Deserialize<'de>>(&self, aggregate_id: Ulid, aggregate_type: &str) -> Result<Vec<EventEnvelope<T, E>>, String> {
        ...
        let rows = statement.query_map(named_params! {
            ...
            let envelope: EventEnvelope<T, E> = serde_json::from_str(&event_json)
    ...
    fn get_all_events<T, E: Event<T> + Serialize + for<'de> Deserialize<'de>>(&self) -> Result<Vec<EventEnvelope<T, E>>, String> {
        let conn = self.pool.get().expect("Failed to get connection");
        let mut statement = conn.prepare("SELECT sequence_number, aggregate_id, aggregate_type, event FROM events ORDER BY sequence_number")
            .map_err(|e| format!("Failed to prepare statement: {}", e))?;
